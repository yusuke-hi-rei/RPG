...
def main():
    ...
    while True:
        ...
        if idx == 0:
            # タイトル画面
            main_title(screen, key)
        elif idx == 1:
            # プレイヤーの移動
            main_movePlayer(screen, key)
        elif idx == 2:
            # 画面切り替え
            main_switchScreen(screen, key)
        elif idx == 3:
            # アイテム入手もしくはトラップ
            main_itemAndTrap(screen, key)
        elif idx == 9:
            # ゲームオーバー
            main_gameOver(screen, key, se)
        elif idx == 10:
            # 戦闘開始
            main_battleStart(screen, key)
        elif idx == 11:
            # プレイヤーのターン(入力待ち)
            ...
            # 逃げられる？
            main_running(screen, key)
        elif idx == 15:
            # 敗北
            main_lose(screen, key)
        elif idx == 16:
            # 勝利
            main_win(screen, key, se)
        elif idx == 17:
            # レベルアップ
            main_levup(screen, key, se)
        elif idx == 20:
            # Potion
            main_potion(screen, key, se)
        elif idx == 21:
            # Blaze gem
            main_blazeGem(screen, key, se)
        elif idx == 22:
            # 戦闘終了
            main_battleEnd(screen, key)
...
"""
 main プレイヤーの移動
"""
def main_movePlayer(screen, key):
    global speed, floor, welcome
...
"""
 main 画面切り替え
"""
def main_switchScreen(screen, key):
...
"""
 main アイテム入手もしくはトラップ
"""
def main_itemAndTrap(screen, key):
    global tmr, idx
...
"""
 main ゲームオーバー
"""
def main_gameOver(screen, key, se):
    global tmr, pl_a, idx
...
　戦闘開始
"""
def main_battleStart(screen, key):
    global tmr, idx
...
"""
 勝利
"""
def main_win(screen, key, se):
    global tmr, idx, emy_lifemax, pl_lifemax
...
"""
 レベルアップ
"""
def main_levup(screen, key, se):
    global lif_p, str_p
    global tmr, idx, emy_lifemax, pl_lifemax, pl_str
...
"""
 Potion
"""
def main_potion(screen, key, se):
    global tmr, idx, pl_lifemax, pl_life, potion
...
"""
 Blaze gem
"""
def main_blazeGem(screen, key, se):
    global dmg
    global tmr, idx, blazegem
